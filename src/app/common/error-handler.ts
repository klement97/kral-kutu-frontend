...
export class ErrorHandler {
  ...
  private setErrorMessage(errors: ValidationErrors) {
    if (errors.serverError) {
      ...
      this.message = 'Kjo fushë është e detyrueshme';
    } else if (errors.minlength) {
      this.message = `Gjatësia maksimale është ${errors.minlength.actualLength}/${errors.minlength.requiredLength}`;
    } else if (errors.maxlength) {
      this.message = `Gjatësia minimale është ${errors.maxlength.actualLength}/${errors.maxlength.requiredLength}`;
    } else if (errors.email) {
      this.message = 'Email nuk është i vlefshëm';
    } else if (errors.min) {
      this.message = `Vlera minimale është ${errors.min.min}, actual value is ${errors.min.actual}`;
    } else if (errors.max) {
      this.message = `Vlera maksimale është ${errors.max.max}, actual value is ${errors.max.actual}`;
    } else if (errors.pattern) {
      this.message = 'E pa vlefshme';
    } else {
      this.message = '';
    }
  }
